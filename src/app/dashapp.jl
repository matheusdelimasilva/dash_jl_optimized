...
mutable struct DashApp
    ...
    devtools ::DevTools
    callbacks ::Dict{Symbol, Vector{Callback}}
    inline_scripts ::Vector{String}

    DashApp(root_path, is_interactive, config, index_string, title = "Dash") =
    new(root_path, is_interactive, config, index_string, title, nothing, DevTools(dash_env(Bool, "debug", false)), Dict{Symbol, Vector{Callback}}(), String[])
