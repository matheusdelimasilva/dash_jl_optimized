...
# Parameters
- `prevent_initial_call`: If true, the callback will not be called when the output is first added to the page
- `allow_duplicate`: If true, allows multiple callbacks to target the same output. Default is false to maintain backward compatibility.
...
function callback!(func::Union{Function, ClientsideFunction, String},
     ...
     return _callback!(func, app, CallbackDeps(output, input, state), prevent_initial_call = prevent_initial_call, allow_duplicate = allow_duplicate)
end
...
function callback!(func::Union{Function, ClientsideFunction, String},
     ...
     return _callback!(func, app, CallbackDeps(output, input, state, length(output) > 1), prevent_initial_call = prevent_initial_call, allow_duplicate = allow_duplicate)
end
...
function _callback!(func::Union{Function, ClientsideFunction, String}, app::DashApp, deps::CallbackDeps; prevent_initial_call, allow_duplicate = false)
    ...
    if haskey(app.callbacks, out_symbol)
        if !allow_duplicate
            error("Multiple callbacks can not target the same output. Offending output: $(out_symbol). To resolve this, set `allow_duplicate=true` on duplicate outputs.")
        end
        push!(app.callbacks[out_symbol], callback_obj)
    else
        app.callbacks[out_symbol] = [callback_obj]
    end
end
